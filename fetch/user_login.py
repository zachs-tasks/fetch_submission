...
class UserLogin(Base):
    ...
    ip: Mapped[str] = mapped_column(String(256), primary_key=True)
    ...
    def create_user_login(js_obj: Dict[str]) -> Optional[UserLogin]:
        ...
        return UserLogin(
            ...
            device_type = data_mask.mask(js_obj.get("device_type"), "device_type"),
            ip = js_obj.get("ip"),
            device_id = data_mask.mask(js_obj.get("device_id"), "device_id"),
            locale = js_obj.get("locale"),
            app_version = data_mask.mask(js_obj.get("app_version"), "app_version"),
